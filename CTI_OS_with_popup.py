...
def draw_up_xml_request(path, array):
    """
    Puts together a request in the XML form to CTI_OS service
...
def convert_names(xml_name):
    """
    Converts names in XML name to name in database (eg. StavDat to stav_dat)
...
def convert_names_dict(xml_name, dictionary):
    """
    Converts names in database to name in XML based on special dictionary
    :param xml_name: given names from XML format
    :type xml_name: string
    :param dictionary: special dictionary where are names which are not possible to automatically convert
...
def save_attributes_to_db(response, db_file, state_vector, logging, dictionary):
    """
    1. Controls errors in given list of posidents
    2. Parses XML returned by CTI_OS service into desired parts which will represent database table attributes
    3. Connects to Export_vse.db
    4. Alters table by adding OS_ID column if not exists
    5. Updates attributes for all posidents in SQLITE3 table rows
    :param response: XML file with all POSIDENT attributes
    :type response: string
    :param db_file: path to the database file
    :type db_file: string
    :param state_vector: statistic information
    :type logging: list
    :param logging: object for logging info, error
    :type logging: logging object
    :param dictionary: special dictionary where are names which are not possible to automatically convert
    :type dictionary: dictionary
    ...
    for os in root.findall('.//{http://katastr.cuzk.cz/ctios/types/v2.8}os'):
        ...
        for child in os:
            ...
            if name == '{http://katastr.cuzk.cz/ctios/types/v2.8}osId':
                xml_attributes[child.tag[child.tag.index('}') + 1:]] = os.find(name).text
        ...
        #  Convert xml_names to database_names
        for xml_name, xml_value in xml_attributes.items():
            database_name = convert_names(xml_name)
            if database_name not in col_names:
                database_name = convert_names_dict(xml_name, dictionary)
        ...
        try:
            ...
            cur.close()
            logging.exception('Problem v pripojeni k databazi')
            raise Exception("CONNECTION TO DATABASE FAILED")
...
def make_form(root, fields):
    """
    Makes structure of popup window
    :param root: window
    :type root: tkinter object
    :param fields: name of fields
    :type fields: string
    :returns entries: the entry from user
    :rtype entries: dictionary
    """
    entries = {}
    for field in fields:
        print(field)
        ...
        ent.pack(side=tk.RIGHT,
                 expand=tk.YES,
                 fill=tk.X)
        entries[field] = ent.get()
    return entries


def clicked(entry):
    """
    This process is made after users clicking at Spustit button
    :param entry: the entry from user
    ...
    path = os.path.join(entry[0], 'request.xml')  # xml without posidents
    db = os.path.join(entry[1], 'Export_vse.db')  # db in the same directory as a script file
    posidents = os.path.join(entry[2], 'posidents.txt')  # the list of posidents in text file
    ...
    # dictionary with relevant names in xml and database which cannot be normally transformed
    ...
    if len(posident_array) <= max_num:
        ...
        state_vector = save_attributes_to_db(response, db, state_vector, logging,
                                             dictionary)  # parse and save attributes into DB
...
# ======================================================================================================================
# Program mainline -- nejde nahoru přidat červený řádek, nejde zkontrolovat checkbox a pouze když je zaškrtnuno spusti


def main():

    fields = ('Cesta k Export_vse.db', 'Cesta k request.xml', 'Cesta k posidents.txt')
    root = tk.Tk()
    ents = make_form(root, fields)
    root.title("Vítej v prográmku ČTI OS")
    root.geometry('800x150')

    is_checked = tk.IntVar()
    chk = tk.Checkbutton(root,
                         text='Stažení všech osobních údajů volaných oprávněných subjektů je na straně ČÚZK logováno a monitorováno! \n Mám právní důvody k získávání osobních údajů oprávněných subjektů a mám zákonný důvod na jejich shromažďování',
                         var=is_checked)
    chk.pack(side=tk.LEFT, padx=5, pady=5)

    b1 = tk.Button(root, text='Konec',
                   command=root.quit)
    b1.pack(side=tk.LEFT, padx=5, pady=5)

    if is_checked.get() == 1:

        b2 = tk.Button(root, text='Spustit',
